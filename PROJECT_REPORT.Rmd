...
var_imp_xgb <- read.csv("plots/05c_variable_importance.csv")
...
# Load XGBoost and Neural Network metrics
...
An interactive web application has been developed using R Shiny to allow users to explore model predictions with custom patient inputs. The application provides real-time predictions from all five models (Logistic Regression, CART, Random Forest, XGBoost, and Neural Network), along with visualizations and model comparison metrics. Access the interactive dashboard at: \url{https://petermangoro.shinyapps.io/hospitalreadmission/}
...
XGBoost (Extreme Gradient Boosting) is an advanced ensemble method that builds upon gradient boosting. It uses a more regularized model formalization to control overfitting, making it robust and efficient. XGBoost sequentially builds decision trees, where each new tree corrects the errors of the previous ones, ultimately creating a strong predictive model.

## Model Training

```{r xgb-model, echo=FALSE, comment=""}
# Load XGBoost model
if(file.exists("model_xgb.RData")) {
  ...
  cat("XGBoost model not found. Please run save_models.R first.\n")
}
```

## Model Evaluation

```{r xgb-evaluation, echo=FALSE, results='asis'}
kable(metrics_xgb, caption = "XGBoost Model Performance Metrics", row.names = FALSE, digits = 2) %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 9)
```

## Confusion Matrix

```{r confusion-matrix-xgb, echo=FALSE, comment=""}
...
if(exists("model_xgb")) {
  ...
  predictions_prob_xgb <- predict(model_xgb, newdata = dtest_xgb)
  ...
  predicted_labels_xgb <- ifelse(predictions_class_xgb == "Readmitted", "Yes", "No")
  actual_labels_xgb <- ifelse(data_test_cart$readmitted == "Readmitted", "Yes", "No")
  conf_matrix_xgb <- table(Predicted = predicted_labels_xgb, Actual = actual_labels_xgb)
  
  # Reorder to show Yes first, then No
  conf_matrix_xgb <- conf_matrix_xgb[c("Yes", "No"), c("Yes", "No")]
  
  # Create a data frame with clear labels
  conf_matrix_xgb_df <- as.data.frame.matrix(conf_matrix_xgb)
  conf_matrix_xgb_df$Predicted <- rownames(conf_matrix_xgb_df)
  conf_matrix_xgb_df <- conf_matrix_xgb_df[, c("Predicted", "Yes", "No")]
  
  # Add totals row
  conf_matrix_xgb_df <- rbind(conf_matrix_xgb_df, 
                              data.frame(Predicted = "Total", 
                                         Yes = sum(conf_matrix_xgb_df$Yes), 
                                         No = sum(conf_matrix_xgb_df$No)))
  
  # Rename columns to show "Actual"
  colnames(conf_matrix_xgb_df) <- c("Predicted", "Actual: Yes", "Actual: No")
  
  kable(conf_matrix_xgb_df, caption = "Confusion Matrix: XGBoost Model - Predicted vs. Actual Readmission Status", 
        row.names = FALSE, align = "lcc") %>%
    kable_styling(latex_options = c("striped", "hold_position"), font_size = 9) %>%
    column_spec(1, width = "3cm", bold = TRUE)
}
...
```{r xgb-importance, echo=FALSE, results='asis'}
kable(head(var_imp_xgb, 10), caption = "Top 10 Most Important Variables in XGBoost Model", row.names = FALSE, digits = 2) %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 9)
```

## ROC Curve

```{r roc-xgb, fig.cap=paste("ROC Curve: XGBoost Model (AUC =", round(auc_xgb$Value, 3), ")"), echo=FALSE, comment="", out.width="0.7\\textwidth", fig.pos="H"}
if(exists("model_xgb") && exists("predictions_prob_xgb")) {
  actual_numeric_xgb <- ifelse(data_test_cart$readmitted == "Readmitted", 1, 0)
  roc_obj_xgb <- roc(actual_numeric_xgb, predictions_prob_xgb)
  auc_value_xgb_plot <- auc(roc_obj_xgb)
  
  cat("Area Under the Curve (AUC): ", round(as.numeric(auc_value_xgb_plot), 4), "\n")
  ...
  p_roc_xgb <- ggplot(roc_data_xgb, aes(x = FPR, y = TPR)) +
    ...
    labs(title = "ROC Curve: XGBoost Model",
         subtitle = paste("AUC =", round(as.numeric(auc_value_xgb_plot), 4)),
         x = "False Positive Rate (1 - Specificity)",
         y = "True Positive Rate (Sensitivity)") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
...
Neural Networks are non-linear models inspired by biological neural networks. They consist of interconnected nodes (neurons) organized in layers that can learn complex patterns in data. For this binary classification task, we use a feedforward neural network with one hidden layer, which can capture non-linear relationships between features and the target variable.

## Model Training

```{r nn-model, echo=FALSE, comment=""}
# Load Neural Network model
if(file.exists("model_nn.RData")) {
  load("model_nn.RData")
  ...
  cat("Neural Network model not found. Please run save_models.R first.\n")
}
```

## Model Evaluation

```{r nn-evaluation, echo=FALSE, results='asis'}
kable(metrics_nn, caption = "Neural Network Model Performance Metrics", row.names = FALSE, digits = 2) %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 9)
```

## Confusion Matrix

```{r confusion-matrix-nn, echo=FALSE, comment=""}
# Make predictions if model is loaded
if(exists("model_nn")) {
  ...
  # Convert to Yes/No labels
  predicted_labels_nn <- ifelse(predictions_class_nn == "Readmitted", "Yes", "No")
  actual_labels_nn <- ifelse(data_test_cart$readmitted == "Readmitted", "Yes", "No")
  conf_matrix_nn <- table(Predicted = predicted_labels_nn, Actual = actual_labels_nn)
  
  # Reorder to show Yes first, then No
  conf_matrix_nn <- conf_matrix_nn[c("Yes", "No"), c("Yes", "No")]
  
  # Create a data frame with clear labels
  conf_matrix_nn_df <- as.data.frame.matrix(conf_matrix_nn)
  conf_matrix_nn_df$Predicted <- rownames(conf_matrix_nn_df)
  conf_matrix_nn_df <- conf_matrix_nn_df[, c("Predicted", "Yes", "No")]
  
  # Add totals row
  conf_matrix_nn_df <- rbind(conf_matrix_nn_df, 
                            data.frame(Predicted = "Total", 
                                       Yes = sum(conf_matrix_nn_df$Yes), 
                                       No = sum(conf_matrix_nn_df$No)))
  
  # Rename columns to show "Actual"
  colnames(conf_matrix_nn_df) <- c("Predicted", "Actual: Yes", "Actual: No")
  
  kable(conf_matrix_nn_df, caption = "Confusion Matrix: Neural Network Model - Predicted vs. Actual Readmission Status", 
        row.names = FALSE, align = "lcc") %>%
    kable_styling(latex_options = c("striped", "hold_position"), font_size = 9) %>%
    column_spec(1, width = "3cm", bold = TRUE)
}
...
```{r roc-nn, fig.cap=paste("ROC Curve: Neural Network Model (AUC =", round(auc_nn$Value, 3), ")"), echo=FALSE, comment="", out.width="0.7\\textwidth", fig.pos="H"}
if(exists("model_nn") && exists("predictions_prob_nn")) {
  actual_numeric_nn <- ifelse(data_test_cart$readmitted == "Readmitted", 1, 0)
  roc_obj_nn <- roc(actual_numeric_nn, as.vector(predictions_prob_nn))
  auc_value_nn_plot <- auc(roc_obj_nn)
  
  cat("Area Under the Curve (AUC): ", round(as.numeric(auc_value_nn_plot), 4), "\n")
  ...
  p_roc_nn <- ggplot(roc_data_nn, aes(x = FPR, y = TPR)) +
    ...
    labs(title = "ROC Curve: Neural Network Model",
         subtitle = paste("AUC =", round(as.numeric(auc_value_nn_plot), 4)),
         x = "False Positive Rate (1 - Specificity)",
         y = "True Positive Rate (Sensitivity)") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
...
comparison_features <- data.frame(
  ...
  XGBoost = c(top5_xgb$Variable[seq_len(n_xgb)], rep("", max(0, 5-n_xgb))),
  Neural_Network = rep("N/A", 5)  # Neural networks don't have direct variable importance
)

kable(comparison_features, caption = "Top 5 Most Important Predictors by Model", row.names = FALSE, align = "lccccc") %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 9)
...
justification_perf <- data.frame(
  ...
  XGBoost = c(
    paste0(round(metrics_xgb$Percentage[metrics_xgb$Metric == "Accuracy"], 1), "%"),
    paste0(round(auc_xgb$Value, 3)),
    ...
    paste0(round(metrics_xgb$Percentage[metrics_xgb$Metric == "F1-Score"], 2), "%")
  ),
  Neural_Network = c(
    paste0(round(metrics_nn$Percentage[metrics_nn$Metric == "Accuracy"], 1), "%"),
    paste0(round(auc_nn$Value, 3)),
    ...
    paste0(round(metrics_nn$Percentage[metrics_nn$Metric == "F1-Score"], 2), "%")
  )
)
...
4. **Clinical Integration**:
   - Develop user-friendly risk calculator (implemented as interactive Shiny app: \url{https://petermangoro.shinyapps.io/hospitalreadmission/})
